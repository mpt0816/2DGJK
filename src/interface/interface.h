...
template<class T>
bool Collision<T>::DetectObstacle(
  ...
  if (!check_trailer) {
    return CheckVehicleModel(path, obstacles, truck_param, check_s, interval, idx_path, idx_obj);
  } else {
    return CheckTruckModel(path, obstacles, truck_param, trailer_yaw, check_s, interval, idx_path, idx_obj);
  }
}
...
bool Collision<T>::DetectObstacle(
  ...
  if (!check_trailer) {
    return CheckVehicleModel(head_pose, obstacles, truck_param, idx_obj);
  } else {
    return CheckTruckModel(head_pose, obstacles, truck_param, trailer_yaw, idx_obj);
  }
}

// return distance between truck and obstacle
template<class T>
double Collision<T>::DetectObstacle(
    const T& head_pose,
    const gjk::Polygon& obstacle,
    const TruckParam& truck_param,
    const bool& check_trailer,
...
template<class T>
bool Collision<T>::CheckVehicleModel(
    const std::vector<T>& path,
    const std::vector<gjk::Polygon>& obstacles,
    const TruckParam& truck_param,
    const double& check_s,
...
template<class T>
bool Collision<T>::CheckTruckModel(
    const std::vector<T>& path,
    const std::vector<gjk::Polygon>& obstacles,
    const TruckParam& truck_param,
    const double& trailer_yaw,
...
template<class T>
void Collision<T>::GenerateVehicleModel(
    const TruckParam& truck_param) {
...
template<class T>
void Collision<T>::GenerateTruckModel(
    const TruckParam& truck_param) {
...
template<class T>
double Collision<T>::AABBDistance(
    const gjk::Polygon& obstacle, 
    const VehicleModel& vehicle) {
  double p_min_x = std::numeric_limits<double>::max();
  double p_max_x = std::numeric_limits<double>::lowest();
